...
namespace
{
	function<void(function<void(optional<int>)>)> numerator(int from, int to)
	{
		return[i = from, to = to](function<void(optional<int>)> callback) mutable {
			callback(i < to ? optional<int>(i++) : nullopt);
		};
	};
	...
	TEST(LAsync, SyncSlotTest)
	{
		...
		loop test_loop([
			source_stream = join(numerator(1, 7), tree_iterator(root)),
